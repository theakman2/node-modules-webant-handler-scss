...
var tests = {
	"test scss 1":function(assert,done) {
	...
	"test scss 2":function(assert,done) {
		var handler = createHandler(Handler);
		var data = {
	...
	"test scss correct file type":function(assert,done) {		
		var handler = createHandler(Handler);
		var data = {
			filePath:__dirname+"/styles.scss",
			requireType:"comment"
		};
	...
	"test scss correct file type with compression (2)":function(assert,done) {		
	...
	"test scss correct file type with compression and as javascript":function(assert,done) {		
		var handler = createHandler(Handler,{compress:true});
		var data = {
			filePath:__dirname+"/styles.scss",
			requireType:"function"
		...
		handler.handle(data,function(resp){
			assert.deepEqual(
				...
				{
					type:"internalJs",
					content:'require("@@css/addStylesheet")(".foo .bar {color:#eeccdd;}");'
				},
				"Handler should call 'update' correctly."
			);
			done();
